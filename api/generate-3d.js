...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!image || typeof image !== 'string' || !image.startsWith('data:image/')) return res.status(400).json({ error: 'image is required as a data URL' });
    ...
    if (!assetResponse.ok) return res.status(assetResponse.status).send(assetText);
    const asset = JSON.parse(assetText);
    if (!asset.assetId || !asset.uploadUrl) return res.status(502).json({ error: 'NVIDIA asset service returned no upload URL.', details: asset });

    const uploadResponse = await fetch(asset.uploadUrl, {
      method: 'PUT',
      headers: { 'Content-Type': contentType, 'x-amz-meta-nvcf-asset-description': 'Hanys reference image' },
      body: bytes
    });
    if (!uploadResponse.ok) return res.status(502).json({ error: 'Nie udało się wysłać obrazu do NVIDIA Asset Storage.', details: await uploadResponse.text() });
    ...
    const nvidiaResponse = await fetch(NVIDIA_URL, {
      ...
      body: JSON.stringify({
        ...
        output_format: 'glb', samples: 1, seed: 0, no_texture: false,
        slat_cfg_scale: 7.5, ss_cfg_scale: 7.5,
        slat_sampling_steps: 50, ss_sampling_steps: 50
      })
    });
