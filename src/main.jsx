...
function Scene({ action, setReady }) {
  return <Canvas shadows camera={{ position: [0, 0.3, 4.2], fov: 38 }} onCreated={() => setReady(true)}>
    ...
    <Suspense fallback={null}><HanysModel action={action} /></Suspense>
...
function App() {
  ...
  useEffect(() => {
    fetch('/models/hanys.glb', { method: 'HEAD' })
      .then((r) => setModelError(!r.ok))
      .catch(() => setModelError(true));
  }, []);

  const react = (next) => { setAction(next); window.setTimeout(() => setAction('idle'), next === 'jump' ? 900 : 1200); };
  ...
  return <main className="app">
    ...
    <section className="actions">
      <button onClick={() => react('happy')}>👋 Przywitaj się</button><button onClick={() => react('jump')}>🦘 Skok</button><button onClick={() => react('happy')}>❤️ Pogłaszcz</button><button onClick={() => react('talk')}>🎤 Porozmawiaj</button>
    </section>
